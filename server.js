...
//our custom dropbox selector to also select users on dropbox-email
Accounts.externalServiceSelectorDropbox = function(
  serviceName, serviceData, options){
  var serviceIdKey = "services." + serviceName + ".id";
  var selector = {};
  selector["$or"] = [{},{}];
...
Meteor.methods({
  connectUserWithDropbox: function (token, secret) {
    // console.log('dropbox connect', token, secret, this.userId)
    //errors
    if (! this.userId)
      throw new Meteor.Error(403, "user must be loggedin");

    var user = Meteor.user();
    if (user.services && user.services.dropbox)
      throw new Meteor.Error(403, "user can not have a dropbox connected account");

    if (Meteor.isServer) {
      var dropboxData = DropboxOAuth.retrieveCredential(token, secret);

      if(!dropboxData)
        throw new Meteor.Error(403, "not able to retreive dropbox data");

      //check if no accounts exists for this dropbox user
      var existing = Meteor.users.find({'services.dropbox.id': dropboxData.serviceData.id}).count();
      if(existing)
        throw new Meteor.Error(403, "user can not have a dropbox connected account");

      Meteor.users.update(this.userId, {$set: {'services.dropbox': dropboxData.serviceData}});
    }
    return dropboxData.serviceData;
  }
});
